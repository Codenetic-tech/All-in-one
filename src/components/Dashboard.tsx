...
const downloadMCXGlobeFile = () => {
  ...
  // Filter for net short positions (netDiff < 0) AND positions that should be allocated
  const allocateData = processedData.data.filter(row => {
    if (row.netDiff >= 0) return false;
    
    // Check if any segment has funds available for allocation
    // We allocate to CO segment only if other segments don't have funds
    const hasCmFunds = row.nseCmBalance > 0;
    const hasFoFunds = row.nseFoBalance > 0;
    const hasCdFunds = row.nseCdsBalance > 0;
    
    // If any segment has funds, don't allocate to CO (move to deallocation instead)
    return !(hasCmFunds || hasFoFunds || hasCdFunds);
  });

  if (allocateData.length === 0) {
    toast({
      title: "No Allocation Positions",
      description: "There are no positions requiring allocation to CO segment.",
      variant: "destructive",
    });
    return;
  }
  ...
  // Generate file content with filtered data - ALLOCATION to CO
  const mcxContent = allocateData.map(row => {
  ...
  const url = window.URL.createObjectURL(blob); // Fixed: createObjectURL instead of revokeObjectURL
  ...
  toast({
    title: "MCX Allocation File Downloaded",
    description: `Generated file with ${allocateData.length} allocation positions to CO segment`,
  });
};

const downloadExcessGlobeFile = () => {
  if (!processedData || processedData.data.length === 0) return;

  // Filter for positions that need deallocation
  const deallocateData = processedData.data.filter(row => {
    // Include netDiff > 0 (excess) OR netDiff < 0 with funds in other segments
    if (row.netDiff > 0) return true;
    
    if (row.netDiff < 0) {
      // Check if any segment has allocated funds that can be deallocated
      const hasCmAllocation = row.cm > 0;
      const hasFoAllocation = row.fo > 0;
      const hasCdAllocation = row.cd > 0;
      
      // If any segment has allocated funds, we deallocate from that segment
      return hasCmAllocation || hasFoAllocation || hasCdAllocation;
    }
    
    return false;
  });

  if (deallocateData.length === 0) {
    toast({
      title: "No Deallocation Positions",
      description: "There are no positions requiring deallocation.",
      variant: "destructive",
    });
    return;
  }
  ...
  // Generate file content with filtered data - DEALLOCATION
  const deallocateRows = [];
  
  deallocateData.forEach(row => {
    let remainingAmount = row.netDiff > 0 ? row.netDiff : 0;
    
    // For netDiff > 0: Deallocate in priority order CM -> FO -> CD -> CO
    if (row.netDiff > 0) {
      // Check CM allocation first
      if (row.cm > 0 && remainingAmount > 0) {
        const amount = Math.min(remainingAmount, row.cm);
        deallocateRows.push(`${getCurrentDate()},CM,M50302,90221,,${row.ucc},C,${amount},,,,,,,D`);
        remainingAmount -= amount;
      }
      
      // Check FO allocation
      if (row.fo > 0 && remainingAmount > 0) {
        const amount = Math.min(remainingAmount, row.fo);
        deallocateRows.push(`${getCurrentDate()},FO,M50302,90221,,${row.ucc},C,${amount},,,,,,,D`);
        remainingAmount -= amount;
      }
      
      // Check CD allocation
      if (row.cd > 0 && remainingAmount > 0) {
        const amount = Math.min(remainingAmount, row.cd);
        deallocateRows.push(`${getCurrentDate()},CD,M50302,90221,,${row.ucc},C,${amount},,,,,,,D`);
        remainingAmount -= amount;
      }
      
      // Remaining amount goes to CO
      if (remainingAmount > 0) {
        deallocateRows.push(`${getCurrentDate()},CO,M50302,90221,,${row.ucc},C,${remainingAmount},,,,,,,D`);
      }
    } 
    // For netDiff < 0 with allocations in other segments: Deallocate all allocations from other segments
    else if (row.netDiff < 0) {
      // Deallocate from CM if allocated
      if (row.cm > 0) {
        deallocateRows.push(`${getCurrentDate()},CM,M50302,90221,,${row.ucc},C,${row.cm},,,,,,,D`);
      }
      
      // Deallocate from FO if allocated
      if (row.fo > 0) {
        deallocateRows.push(`${getCurrentDate()},FO,M50302,90221,,${row.ucc},C,${row.fo},,,,,,,D`);
      }
      
      // Deallocate from CD if allocated
      if (row.cd > 0) {
        deallocateRows.push(`${getCurrentDate()},CD,M50302,90221,,${row.ucc},C,${row.cd},,,,,,,D`);
      }
    }
  });

  const deallocateContent = deallocateRows.join('\n');
  ...
  const fullContent = header + deallocateContent;

  const blob = new Blob([fullContent], { type: 'text/plain' });
  const url = window.URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `90221_ALLOC_${dateString}.T0160`;
  link.click();
  window.URL.revokeObjectURL(url);

  toast({
    title: "Deallocation File Downloaded",
    description: `Generated file with ${deallocateRows.length} deallocation rows`,
  });
};
            ...
            <div className="flex flex-col sm:flex-row gap-2 flex-wrap">
              ...
              >
                ...
                MCX Globe
                            ...
                            <Button
                onClick={downloadExcessGlobeFile}
